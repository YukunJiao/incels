...
# ====================================================================
# SECTION 8: conText EMBEDDING REGRESSION (interaction covariates)
# For each group (incel / normie / women), regress the ALC embedding
# on interaction-quality docvars to get coefficient direction vectors.
# ====================================================================

tar_target(
  toks_main_interaction,
  {
    toks <- toks_main
    dv   <- quanteda::docvars(toks)
    quanteda::docvars(toks, "is_comment")          <- as.integer(dv$doc_type            == "comment")
    quanteda::docvars(toks, "nested_reply")        <- as.integer(dv$reply_type          == "nested_comment")
    quanteda::docvars(toks, "is_active_author")    <- as.integer(dv$author_activity_bin == "21+")
    quanteda::docvars(toks, "score_top10")         <- as.integer(dv$score_tier_ym       == "top10")
    quanteda::docvars(toks, "score_bottom10")      <- as.integer(dv$score_tier_ym       == "bottom10")
    quanteda::docvars(toks, "large_thread")        <- as.integer(dv$thread_comment_bin  == "51+")
    quanteda::docvars(toks, "many_direct_replies") <- as.integer(dv$direct_child_bin    == "6+")
    toks
  }
),

tar_target(
  toks_main_interaction_time,
  {
    toks <- toks_main_interaction
    dv   <- quanteda::docvars(toks)
    quanteda::docvars(toks, "time_window") <- factor(
      dplyr::case_when(
        as.character(dv$ym) >= "2016-07" & as.character(dv$ym) < "2017-01" ~ "2016_07_to_2016_12",
        as.character(dv$ym) >= "2017-01" & as.character(dv$ym) <= "2017-10" ~ "2017_01_to_2017_10",
        TRUE ~ NA_character_
      ),
      levels = c("2016_07_to_2016_12", "2017_01_to_2017_10")
    )
    toks
  }
),
...
tar_target(
  toks_main_interaction_time_x,
  {
    toks <- toks_main_interaction_time
    dv   <- quanteda::docvars(toks)
    quanteda::docvars(toks, "time_window_2017_01_to_2017_10") <-
      as.integer(dv$time_window == "2017_01_to_2017_10")
    ...
    for (term in interaction_terms) {
      quanteda::docvars(toks, paste0("time_x_", term)) <-
        quanteda::docvars(toks, "time_window_2017_01_to_2017_10") *
        quanteda::docvars(toks, term)
    }
...
tar_target(
  context_incel_time_x,
  conText::conText(
    formula           = c("incel", "incels") ~ time_window_2017_01_to_2017_10 + nested_reply + score_top10 + score_bottom10 + large_thread + many_direct_replies + time_x_nested_reply + time_x_score_top10 + time_x_score_bottom10 + time_x_large_thread + time_x_many_direct_replies,
    data              = toks_main_interaction_time_x,
    pre_trained       = local_glove,
    transform         = TRUE, transform_matrix = local_transform,
...
tar_target(
  context_normie_time_x,
  conText::conText(
    formula           = c("normie", "normies") ~ time_window_2017_01_to_2017_10 + nested_reply + score_top10 + score_bottom10 + large_thread + many_direct_replies + time_x_nested_reply + time_x_score_top10 + time_x_score_bottom10 + time_x_large_thread + time_x_many_direct_replies,
    data              = toks_main_interaction_time_x,
    pre_trained       = local_glove,
    transform         = TRUE, transform_matrix = local_transform,
    jackknife         = FALSE, confidence_level = 0.95,
...
tar_target(
  context_women_time_x,
  conText::conText(
    formula           = c("women", "woman", "females", "female", "girl", "girls") ~ time_window_2017_01_to_2017_10 + nested_reply + score_top10 + score_bottom10 + large_thread + many_direct_replies + time_x_nested_reply + time_x_score_top10 + time_x_score_bottom10 + time_x_large_thread + time_x_many_direct_replies,
    data              = toks_main_interaction_time_x,
    pre_trained       = local_glove,
    transform         = TRUE, transform_matrix = local_transform,
    jackknife         = FALSE, confidence_level = 0.95,
...
#-----
tar_target(
  context_time_x_scenario_proj,
  {
    models <- list(
      incel  = context_incel_time_x,
      normie = context_normie_time_x,
      women  = context_women_time_x
    )
    
    scenarios <- tibble::tribble(
      ~scenario,                  ~term,
      "Baseline",                 NA_character_,
      "Nested reply",             "nested_reply",
      "Top-10% score",            "score_top10",
      "Bottom-10% score",         "score_bottom10",
      "Large thread (51+)",       "large_thread",
      "Many direct replies (6+)", "many_direct_replies"
    )
    
    time_windows <- tibble::tibble(
      time_window = c("2016_07_to_2016_12", "2017_01_to_2017_10")
    )
    
    cosine_projection <- function(v, d) {
      sum(v * d) / (sqrt(sum(v^2)) * sqrt(sum(d^2)))
    }
    
    purrr::imap_dfr(models, function(mod, grp) {
      coef_mat <- as(mod, "matrix")
      intercept <- as.numeric(coef_mat["(Intercept)", ])
      
      tidyr::crossing(time_windows, scenarios) |>
        dplyr::mutate(group = grp) |>
        dplyr::rowwise() |>
        dplyr::mutate(
          embedding = list({
            v <- intercept
            is_late <- time_window == "2017_01_to_2017_10"
            
            if (is_late) {
              v <- v + as.numeric(coef_mat["time_window_2017_01_to_2017_10", ])
            }
            if (!is.na(term)) {
              v <- v + as.numeric(coef_mat[term, ])
            }
            if (is_late && !is.na(term)) {
              v <- v + as.numeric(coef_mat[paste0("time_x_", term), ])
            }
        ...
        tidyr::crossing(
          tibble::tibble(
            dimension = names(dirs),
            direction = unname(dirs)
          )
        ) |>
        dplyr::mutate(
          projection = purrr::map2_dbl(
            embedding,
            direction,
            ~ cosine_projection(.x, as.numeric(.y))
          )
        ) |>
        dplyr::select(group, time_window, scenario, term, dimension, projection)
    })
  }
),
...
tar_target(
  context_time_x_condition_effect,
  {
    baseline <- context_time_x_scenario_proj |>
    ...
    context_time_x_scenario_proj |>
      ...
      dplyr::left_join(
        baseline,
        by = c("group", "time_window", "dimension")
      ) |>
      dplyr::mutate(
        condition_effect = projection - baseline_projection
      )
  }
),
...
tar_target(
  context_time_x_interaction_effect,
  {
    context_time_x_condition_effect |>
      dplyr::select(group, scenario, term, dimension, time_window, condition_effect) |>
      tidyr::pivot_wider(
        id_cols = c(group, scenario, term, dimension),
        names_from = time_window,
        values_from = condition_effect
      ) |>
      dplyr::mutate(
        interaction_effect = `2017_01_to_2017_10` - `2016_07_to_2016_12`
      )
  }
),
#------plot for H5
tar_target(
  plot_context_time_x_condition_effect,
  {
    df <- context_time_x_condition_effect |>
      dplyr::mutate(
        time_window = factor(
          time_window,
          levels = c("2016_07_to_2016_12", "2017_01_to_2017_10"),
          labels = c("2016-07 to 2016-12", "2017-01 to 2017-10")
        ),
        scenario = factor(
          scenario,
          levels = c(
            ...
            "Bottom-10% score",
            "Large thread (51+)",
            "Many direct replies (6+)"
          )
        ),
        group = factor(
          group,
          levels = c("incel", "normie", "women"),
          labels = c("Incel", "Normie", "Women")
        ),
    ...
    ggplot2::ggplot(
      df,
      ggplot2::aes(x = time_window, y = condition_effect, color = group, group = group)
    ) +
      ggplot2::geom_hline(yintercept = 0, linetype = "dashed", color = "gray70") +
      ggplot2::geom_line(linewidth = 0.7) +
      ggplot2::geom_point(size = 1.9) +
      ggplot2::facet_grid(scenario ~ dimension) +
      ggplot2::labs(
        x = NULL,
        y = "Condition effect relative to baseline",
        color = NULL
      ) +
...
tar_target(
  plot_context_time_x_interaction_effect,
  {
    df <- context_time_x_interaction_effect |>
      dplyr::mutate(
        scenario = factor(
          scenario,
          levels = c(
    ...
    ggplot2::ggplot(
      df,
      ggplot2::aes(x = interaction_effect, y = scenario, color = group)
    ) +
...
tar_target(
  context_time_x_alc_vectors_check,
  {
    models <- list(
      incel  = context_incel_time_x,
      normie = context_normie_time_x,
    ...
    purrr::imap_dfr(models, function(mod, grp) {
      ...
      tidyr::crossing(time_windows, scenarios) |>
        ...
        dplyr::ungroup() |>
        dplyr::mutate(
          alc_norm = purrr::map_dbl(alc_vector, ~ sqrt(sum(.x^2)))
        ) |>
        dplyr::select(group, time_window, scenario, term, alc_norm, alc_vector)
    })
  }
),

tar_target(
  context_time_x_alc_projection_check,
  {
    cosine_projection <- function(v, d) {
      sum(v * d) / (sqrt(sum(v^2)) * sqrt(sum(d^2)))
    }
    ...
    context_time_x_alc_vectors_check |>
      ...
      dplyr::mutate(
        projection_check = purrr::map2_dbl(
          alc_vector,
          direction,
          ~ cosine_projection(.x, as.numeric(.y))
        )
      ...
      dplyr::select(
        group, time_window, scenario, term, dimension, alc_norm, projection_check
      )
  }
),

tar_target(
  context_time_x_alc_projection_compare,
  {
    context_time_x_alc_projection_check |>
      dplyr::left_join(
        context_time_x_scenario_proj |>
          dplyr::rename(projection_existing = projection),
        by = c("group", "time_window", "scenario", "term", "dimension")
      ) |>
      dplyr::mutate(
        diff = projection_check - projection_existing,
        abs_diff = abs(diff)
      )
  }
),
...
tar_target(
  context_time_x_alc_projection_compare_summary,
  {
    context_time_x_alc_projection_compare |>
      dplyr::summarise(
        n = dplyr::n(),
        ...
        p95_abs_diff = stats::quantile(abs_diff, 0.95, na.rm = TRUE),
        .groups = "drop"
      )
  }
),
...
tar_target(
  context_time_x_conText_uncertainty_slots,
  {
    models <- list(
      incel  = context_incel_time_x,
      normie = context_normie_time_x,
    ...
    purrr::imap_dfr(models, function(mod, grp) {
      purrr::map_dfr(methods::slotNames(mod), function(slot_name) {
        obj <- methods::slot(mod, slot_name)
        tibble::tibble(
          group = grp,
          slot = slot_name,
          ...
          looks_like_uncertainty = grepl(
            "perm|jack|boot|sample|replicate|se|std|ci|conf",
            slot_name,
            ignore.case = TRUE
          )
        )
...
tar_target(
  pipeline_packages,
  c("quanteda", "conText","text2vec", "text2map")
),
tar_target(
  bib,
  knitr::write_bib(pipeline_packages, "css-thesis/references/r-references.bib")
),

tarchetypes::tar_quarto(
  manuscript,
  "css-thesis/manuscript_Yukun_Jiao.qmd",
  quiet = FALSE
),
